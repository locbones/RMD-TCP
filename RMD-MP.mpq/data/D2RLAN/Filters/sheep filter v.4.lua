--- Filter Title: Sheep's Base Filter v.4
--- Filter Type: (Generic base filter)
--- Filter Description: A generic base filter looking to keep the Diablo II aesthetic.\nFocuses mainly on notifying you of good drops and hiding complete junk.\nRule 1: Hides all 1 socket superior or lower items.(these items are pretty useless)\nRule 2: Hides zero skill paladin weapons.\n   Rule 2b: Shows 3 skill scepters in red with text\nRule 3: Hides scrolls on ground\nRule 4: Hides potions on ground (minus rejuvs) after level 25\nRule 5: Adds * * in red and notifies when unique amulets drop\nRule 6: Adds  * * in red and notifies when unique rings drop\nRule 7: Adds  * * in red and notifies when set amulets drop\nRule 8: Adds * * in red and notifies when set rings drop\nRule9-9d:Colors runes and adds notification Ultras get |X| symbol\nRule 10: Unique Archons are colored red and notification\nRule 11: Potentially good uniques (ie hydra master) are color pink\nRule 12:Unique charm notification\nRule 13: Facets notification and renamed added\nRule 14: Treasure Chest/gem vein/Gifts notification\nRule 15: Valuable Uniques notification and $ symbol\nRule 16: good superior bases have |GB| symbol\nRule 17-29: Hide non-superior/non-class bases by level\nRule 30-44:Hide non-superior/non-class magic items by level\nRule 45:Hides magic and lower quivers\nRule 46: Hides gold by amount and level\nRule 47 :Hides non superior items with less than 2 Sockets if the player is above 80 does not include staff mod items\nRule 48:Paladin Shield bases with high resistances (thanks cogy)\nRule 49: Sorting of weapon bases by socket amount(currently only removes 2 socket bases post 80) (thanks elr for sorting all base by sockets )\nRule 50: Shows Necro Shield bases with 3 summon skills\nRule 51:Shows 2 bow skills Grand Matron/matriarchal Bow bases\nRule 52:Shows +3 LF Amazon Javs bases \nRule 53: Shows Raven Damage Druid Helmet bases\n(thanks vivasen for the stat searchers for 50-53)
return {
    reload = "{pink}Sheep's Base Filter v.4 {grey} updated 6/1/25 {Green}Reloaded",
    rules = {
        ...
        ---rule 44:  hiding non-class base magic items by level (weapons leaving runeword bases ie flails,crystal swords)
        {
            ...
            quality = 4,
            pstat = { index = 12, op = ">=", value = 60 }, -- Char Level is >= 40
            hide = true
        },
